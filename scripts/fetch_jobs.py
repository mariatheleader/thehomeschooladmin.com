import requests
import os
from datetime import datetime

def is_maine_eligible(job_text):
    """Check if job allows Maine workers"""
    job_text_lower = str(job_text).lower()
    exclude_locations = [
        "california only", "texas only", "new york only",
        "west coast only", "pst only", "no maine"
    ]
    for location in exclude_locations:
        if location in job_text_lower:
            return False
    return True

# LOOSENED TARGET KEYWORDS (more flexible)
TARGET_KEYWORDS = [
    "enablement", "customer success", "product operations",
    "operations", "implementation", "program manager",
    "learning", "content operations", "education",
    "workflow", "documentation", "training",
    "adoption", "customer education", "onboarding"
]

# TIGHTER EXCLUDE KEYWORDS (only obvious mismatches)
EXCLUDE_KEYWORDS = [
    "data analyst", "data science", "data labeling",
    "software engineer", "backend engineer", "frontend engineer",
    "devops", "infrastructure", "warehouse",
    "delivery driver", "retail", "hospitality"
]

def fetch_remotive_jobs():
    """Fetch from Remotive"""
    jobs = []
    try:
        url = "https://remotive.com/api/remote-jobs"
        response = requests.get(url, params={"limit": 100}, timeout=10)
        data = response.json()
        
        for job in data.get("jobs", []):
            title = (job.get("title") or "").lower()
            description = (job.get("description") or "").lower()
            location = job.get("candidate_required_location", "").lower()
            
            if not any(region in location for region in ["usa", "united states", "north america", "anywhere"]):
                continue
            
            # Must have at least ONE target keyword (flexible matching)
            if not any(target in title or target in description for target in TARGET_KEYWORDS):
                continue
            
            # Must NOT have exclude keywords
            if any(exclude in title or exclude in description for exclude in EXCLUDE_KEYWORDS):
                continue
            
            # Must be Maine eligible
            if not is_maine_eligible(description):
                continue
            
            jobs.append({
                "title": job.get("title", "N/A"),
                "company": job.get("company_name", "N/A"),
                "location": location,
                "salary": job.get("salary") or "Not listed",
                "description": (job.get("description") or "")[:200],
                "url": job.get("url", "#"),
                "company_url": job.get("company_url", ""),
                "source": "Remotive"
            })
    except Exception as e:
        print(f"Remotive error: {e}")
    return jobs

def fetch_github_jobs():
    """Fetch from GitHub Jobs"""
    jobs = []
    try:
        url = "https://api.github.com/repos/github-community/githubjobs/issues"
        response = requests.get(url, timeout=10)
        data = response.json()
        
        for job in data[:100]:
            title = (job.get("title") or "").lower()
            body = (job.get("body") or "").lower()
            
            if "remote" not in body and "work from home" not in body:
                continue
            if "usa" not in body and "united states" not in body:
                continue
            if not any(target in title or target in body for target in TARGET_KEYWORDS):
                continue
            if any(exclude in title or exclude in body for exclude in EXCLUDE_KEYWORDS):
                continue
            if not is_maine_eligible(body):
                continue
            
            jobs.append({
                "title": job.get("title", "N/A"),
                "company": "Check listing",
                "location": "Remote - USA",
                "salary": "Check listing",
                "description": body[:200],
                "url": job.get("html_url", "#"),
                "company_url": "",
                "source": "GitHub Jobs"
            })
    except Exception as e:
        print(f"GitHub Jobs error: {e}")
    return jobs

def fetch_devto_jobs():
    """Fetch from Dev.to"""
    jobs = []
    try:
        url = "https://dev.to/api/articles"
        params = {"tag": "remote", "per_page": 50}
        response = requests.get(url, params=params, timeout=10)
        data = response.json()
        
        for article in data:
            if "job" not in article.get("tag_list", []):
                continue
            
            title = (article.get("title") or "").lower()
            body = (article.get("body_markdown") or "").lower()
            
            if not any(region in body for region in ["usa", "united states"]):
                continue
            if not any(target in title or target in body for target in TARGET_KEYWORDS):
                continue
            if any(exclude in title or exclude in body for exclude in EXCLUDE_KEYWORDS):
                continue
            if not is_maine_eligible(body):
                continue
            
            jobs.append({
                "title": article.get("title", "N/A"),
                "company": "Check listing",
                "location": "Remote - USA",
                "salary": "Check listing",
                "description": article.get("description", "")[:200],
                "url": article.get("url", "#"),
                "company_url": "",
                "source": "Dev.to"
            })
    except Exception as e:
        print(f"Dev.to error: {e}")
    return jobs

def fetch_we_work_remotely_jobs():
    """Fetch from We Work Remotely"""
    jobs = []
    try:
        url = "https://weworkremotely.com/api/v1/remote-jobs"
        response = requests.get(url, timeout=10)
        data = response.json()
        
        for job in data.get("remote_jobs", []):
            title = (job.get("title") or "").lower()
            description = (job.get("description") or "").lower()
            location = job.get("location", "").lower()
            
            if not any(region in location for region in ["usa", "united states"]):
                continue
            if not any(target in title or target in description for target in TARGET_KEYWORDS):
                continue
            if any(exclude in title or exclude in description for exclude in EXCLUDE_KEYWORDS):
                continue
            if not is_maine_eligible(description):
                continue
            
            jobs.append({
                "title": job.get("title", "N/A"),
                "company": job.get("company", "N/A"),
                "location": location,
                "salary": job.get("salary_range") or "Not listed",
                "description": (job.get("description") or "")[:200],
                "url": job.get("url", "#"),
                "company_url": job.get("company_url", ""),
                "source": "We Work Remotely"
            })
    except Exception as e:
        print(f"We Work Remotely error: {e}")
    return jobs

def fetch_remoteok_jobs():
    """Fetch from RemoteOK"""
    jobs = []
    try:
        url = "https://remoteok.com/api"
        response = requests.get(url, timeout=10)
        data = response.json()
        
        for job in data[1:150]:
            title = (job.get("title") or "").lower()
            body = str(job).lower()
            
            if not any(region in body for region in ["usa", "united states"]):
                continue
            if not any(target in title or target in body for target in TARGET_KEYWORDS):
                continue
            if any(exclude in title or exclude in body for exclude in EXCLUDE_KEYWORDS):
                continue
            if not is_maine_eligible(body):
                continue
            
            jobs.append({
                "title": job.get("title", "N/A"),
                "company": job.get("company", "N/A"),
                "location": "Remote",
                "salary": job.get("salary") or "Not listed",
                "description": (job.get("description") or "")[:200],
                "url": job.get("url", "#"),
                "company_url": "",
                "source": "RemoteOK"
            })
    except Exception as e:
        print(f"RemoteOK error: {e}")
    return jobs

def fetch_angellist_jobs():
    """Fetch from AngelList"""
    jobs = []
    try:
        url = "https://api.angel.co/1/jobs"
        params = {"page": 1, "per_page": 50}
        response = requests.get(url, params=params, timeout=10)
        data = response.json()
        
        for job in data.get("jobs", []):
            title = (job.get("title") or "").lower()
            description = (job.get("description") or "").lower()
            
            if "remote" not in description and "remote" not in title:
                continue
            if not any(target in title or target in description for target in TARGET_KEYWORDS):
                continue
            if any(exclude in title or exclude in description for exclude in EXCLUDE_KEYWORDS):
                continue
            
            jobs.append({
                "title": job.get("title", "N/A"),
                "company": job.get("startup", {}).get("name", "N/A"),
                "location": "Remote",
                "salary": "Not listed",
                "description": description[:200],
                "url": job.get("url", "#"),
                "company_url": "",
                "source": "AngelList"
            })
    except Exception as e:
        print(f"AngelList error: {e}")
    return jobs

def fetch_all_jobs():
    """Fetch from ALL free sources"""
    all_jobs = []
    
    print("Fetching from all free job boards...")
    all_jobs.extend(fetch_remotive_jobs())
    all_jobs.extend(fetch_github_jobs())
    all_jobs.extend(fetch_devto_jobs())
    all_jobs.extend(fetch_we_work_remotely_jobs())
    all_jobs.extend(fetch_remoteok_jobs())
    all_jobs.extend(fetch_angellist_jobs())
    
    # Deduplicate by URL
    seen = set()
    unique_jobs = []
    for job in all_jobs:
        if job["url"] not in seen:
            seen.add(job["url"])
            unique_jobs.append(job)
    
    print(f"Found {len(unique_jobs)} total jobs")
    return unique_jobs  # Return ALL jobs, no limit

def send_email_sendgrid(jobs):
    """Send via SendGrid"""
    
    api_key = os.getenv("SENDGRID_API_KEY")
    sender_email = "maria@thehomeschooladmin.com"
    
    html = f"""
    <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 700px;">
            <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">
                📧 USA Async Remote Jobs (Maine Eligible)
            </h2>
            <p style="font-size: 16px; color: #555;">
                <strong>{len(jobs)}</strong> opportunities from free job boards today.
            </p>
    """
    
    if jobs:
        for i, job in enumerate(jobs, 1):
            company_link = f'<a href="{job["company_url"]}" style="color: #0645ad;">{job["company"]}</a>' if job["company_url"] else job["company"]
            
            html += f"""
            <div style="margin-bottom: 20px; padding: 15px; background: #f8f9fa; border-left: 4px solid #3498db; border-radius: 4px;">
                <p style="margin: 0 0 8px 0;">
                    <strong style="font-size: 16px;">{i}. {job['title']}</strong>
                </p>
                <p style="margin: 0 0 5px 0; color: #555;">
                    <strong>Company:</strong> {company_link}
                </p>
                <p style="margin: 0 0 5px 0; color: #555;">
                    <strong>Location:</strong> {job['location']} | <strong>Salary:</strong> {job['salary']}
                </p>
                <p style="margin: 0 0 10px 0; color: #666; font-size: 14px;">
                    {job['description']}
                </p>
                <p style="margin: 0;">
                    <a href="{job['url']}" style="display: inline-block; padding: 8px 14px; background: #3498db; color: white; text-decoration: none; border-radius: 4px;">
                        Apply →
                    </a>
                </p>
                <p style="margin: 5px 0 0 0; font-size: 11px; color: #999;">
                    {job['source']}
                </p>
            </div>
            """
    else:
        html += "<p style='color: #999;'>No matching jobs found today.</p>"
    
    html += """
        </body>
    </html>
    """
    
    url = "https://api.sendgrid.com/v3/mail/send"
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json"
    }
    
    data = {
        "personalizations": [{
            "to": [{"email": "maria.galyean.work@gmail.com"}]
        }],
        "from": {"email": sender_email},
        "subject": f"🎯 Async Remote Jobs ({len(jobs)} available) - Maine Eligible - {datetime.now().strftime('%B %d')}",
        "content": [{
            "type": "text/html",
            "value": html
        }]
    }
    
    response = requests.post(url, json=data, headers=headers)
    
    if response.status_code == 202:
        print(f"✅ Email sent! {len(jobs)} jobs from 6 free boards")
    else:
        print(f"❌ Error: {response.status_code}")

if __name__ == "__main__":
    jobs = fetch_all_jobs()
    send_email_sendgrid(jobs)
